...
- **故障恢复**：实时扫描与历史补扫分别保存进度，实时扫描落后超过 2000 块时，保存未扫描区间并恢复最近 20 块扫描，旧积压进入补扫队列；失败退避并重试待发送通知。金库注册同样将实时与历史扫描分离；历史节点不可用时保留缺口并退避，不阻塞实时扫描。状态命令分别显示实时延迟、连接状态和历史补扫异常。Factory 启动回扫限制在近期 5000 块内，较早区间由持续补扫处理。
