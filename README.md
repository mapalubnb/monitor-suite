...
- **前端增量抓取**：FourMeme 保留真实 `dpl` 部署 URL，只下载新增或 URL 变化的资源；部分失败会暂存成功结果并在下轮只补抓失败项。
- **前端抗风控**：同一进程使用稳定浏览器标识，脚本和样式请求携带正确的资源类型与页面来源；静态资源异常不会阻塞页面和 API 监控。
...
前端页面或 API 遇到 `403`、`429`、Cloudflare 或网络异常时会自动退避和重试。静态资源单独重试并聚合限频记录，只有当前资源全部就绪后才更新正式快照，不会把半包或请求失败误判为业务变更。
