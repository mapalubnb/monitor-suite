...
- **稳定低延迟**：支持 PM2、动态 RPC 竞速、短超时切换、断点续扫和去重；Factory 名称与飞书发送不阻塞后续扫描。
...
| Flap.sh | Factory 状态事件补扫 | 加权优先后台运行，自动找回实时窗口外的变化 |
| Flap.sh | Factory 完整历史补扫 | 公平轮转后台运行 |
...
- Flap 轮询最低 `500ms`。
- Flap Factory 实时扫描使用固定 1 秒节拍和 0 确认块；RPC 自动选择低延迟健康节点，历史和漏检补扫公平轮转并主动让路。
- Factory 实时、漏检和历史请求可以并行，扫描结果、游标、状态文件和通知队列按单写顺序合并，不会互相覆盖。
...
- Factory 历史进度较慢：检查 `fl-status` 的状态事件游标和 RPC 日志；实时新资产快通道不会等待完整历史补扫。
