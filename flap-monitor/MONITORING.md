...
| `FLAP_EARLY_INTERVAL_MS` | 1000 | 链上扫描完成后的等待间隔 |
| `FLAP_EARLY_COW_INTERVAL_MS` | 10000 | 外部数据轮询目标间隔，实际受请求耗时／配额影响 |
...
| `FLAP_EARLY_WS_ENABLED` | true | 精准钱包/合约订阅唤醒扫描 |
...
链上与外部 API 独立调度；WSS 只唤醒规范链 HTTP 扫描，HTTP 游标负责断点补扫，避免同一回执双通道重复推送。每轮完整窗口校验后提交，失败不越过游标。游标块 hash 改变时回退 128 块，清理关联本地信号，发出重组更正，再重扫。超过该窗口的深重组需要人工回溯；低确认数信号不是最终性承诺。
