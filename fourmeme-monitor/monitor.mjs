...
function buildStartupProgressContent() {
  ...
  return [
    `**📌 启动状态**`,
    `- 状态: ⏳ **监控启动中**`,
    `- 进度: 进程已启动，正在建立/刷新首轮基线`,
    `- 下一步: 首轮完成后会自动更新为运行中状态`,
    ``,
    `**🎯 首轮检查**`,
    `- 前端：当前快照 ${pageCount} 个页面｜每 ${Math.round(CONFIG.intervals.frontend / 1000)} 秒`,
    `- API：每 ${Math.round(CONFIG.intervals.api / 1000)} 秒`,
    `- 模块: 底池 / OpenFour / GitHub / 合约 / 链上参数 / 创建者动作`,
    ``,
    `**🔎 查看方式**`,
    `- 状态: fm-status`,
    `- 日志: fm-log`,
...
function buildStartupReadyContent() {
  ...
  return [
    ...
    `- 前端：${pageCount} 个页面｜每 ${Math.round(CONFIG.intervals.frontend / 1000)} 秒`,
    `- API：${apiCount} 个端点｜每 ${Math.round(CONFIG.intervals.api / 1000)} 秒`,
    `- OpenFour：模板 ${openFourTemplateCount} 个｜模块 ${openFourModuleCount} 个｜${registryTrigger}`,
    `- GitHub：${githubRepoCount} 个仓库｜${githubSha}｜${formatInterval(CONFIG.intervals.github)}`,
    `- 合约/链上：合约 ${contractCount} 个｜AgentNFT ${snapshot?.onchainParams?.agentNftCount ?? "未知"} 个`,
