...
- 实时日志、历史日志、实时区块和历史区块按方法分工；历史状态也可独立配置。历史池留空时回退到对应普通读取池，绝不将完整区块错误路由到日志池。
- 合约历史工作独立于核心检查；网络查询不占用状态变更串行队列，完整结果提交和通知去重仍串行进行。
- 不可用区间按起点记录重试期限并持久化，剩余区间完整返回待补列表，允许先补可服务缺口，不把未扫描区间标记完成。
- 403/429 按方法及日志实时/历史类别冷却；401 和明确账户级错误仍全局冷却。生产问题应结合上游错误与本地预算统计排查。
- 完整区块最多缓存 64 块，复用前重新验证链上区块头；边界重组校验和交易回执校验保留。
- 所有定时频率不变；公共节点服务能力和限额仍需持续观察，不能保证不出现上游错误。

1.10.3：扩展扫描分批让出执行机会给到期的核心和实时事件检查；核心时间戳只由核心成功更新，扩展结果按读取区块防止覆盖更新的数据。
