...
const CONFIG = {
  ...
  actorMonitor: {
    ...
    maxBlocksPerRun: readPositiveIntEnv("FOURMEME_ACTOR_MAX_BLOCKS", 64),
    catchUpMaxBlocksPerRun: readPositiveIntEnv("FOURMEME_ACTOR_CATCHUP_MAX_BLOCKS", 256),
...
function actorScanLimit(lagBlocks) {
  const normalLimit = Math.max(1, CONFIG.actorMonitor.maxBlocksPerRun);
  const catchUpLimit = Math.max(normalLimit, CONFIG.actorMonitor.catchUpMaxBlocksPerRun);
  if (lagBlocks <= normalLimit) return normalLimit;
  return Math.min(catchUpLimit, Math.max(normalLimit, lagBlocks));
}
...
async function runActorCheck() {
  ...
  const lagBlocks = Math.max(0, safeLatest - state.lastBlock);
  const scanLimit = actorScanLimit(lagBlocks);
  const toBlock = Math.min(safeLatest, fromBlock + scanLimit - 1);
  state.latestSafeBlock = safeLatest;
  state.lagBlocks = lagBlocks;
  state.scanLimit = scanLimit;
  if (lagBlocks > CONFIG.actorMonitor.maxBlocksPerRun) {
    log(`[创建者] 区块游标落后 ${lagBlocks} 块，本轮追赶扫描 ${scanLimit} 块（${fromBlock}-${toBlock}）`);
  }
