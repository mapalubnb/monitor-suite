...
- Vault Portal 注册独立轮询并监听 WSS；网页支持 ETag／Last-Modified 条件请求。没有引入付费 Webhook 或降低确认块数。
- 新增 `realtime-scheduler.mjs`，安装脚本同步复制；升级需更新整个项目并重启。详见 [监控说明](flap-monitor/MONITORING.md)。

## 上次更新：1.4.1 / Flap 1.3.1

- 修复资源下载不完整时固化残缺快照、页面／Vault Factory／链上注册消息未送达就推进基线的问题；请求超时覆盖响应正文。
- 修复通知队列静默截断、首次提前扫描失败后丢失初始化状态、CoW 重复开放与过期状态遗漏；限制无关池持续查询，候选容量告警不阻塞扫描。
- CAStore 优先按真实卡片边界解析，兼容 FOMOX、动态空投，排除 FAQ；资源与 i18n 共用有界下载缓存，同一 Safe 批量提案只模拟一次。
- 飞书卡片精简占位说明、重复标题、零数量变更和全零路径扩展字段，增加状态配色，保留完整地址、非零扩展字段及必要的不确定性说明。
- Chrome 与链上核对结果、覆盖限制见 [核对报告](flap-monitor/AUDIT-2026-09-25.md)。未自动部署或发送测试卡片。

## 上次更新：1.4.0 / Flap 1.3.0
