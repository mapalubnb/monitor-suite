...
| Vault Factory | Vault Portal 中的工厂注册与配置变化 |
...
- **启动通知**：进程启动后独立发送单张卡片，后台更新首轮检查进度和异常数量；发送或更新失败会自动重试。详细运行状态使用 `fm-status` 或 `fl-status` 查看。
- **可选 AI 分析**：先推送监控结果，再异步补充摘要。
- **实时与轮询结合**：链上模块使用 WSS 触发及 HTTP 兜底；页面、API、GitHub 和 Safe 服务按各自频率查询。
- **故障恢复**：支持失败退避、状态持久化、链上断点补扫和待发送通知重试。
