...
普通读取、实时日志、历史补扫可以通过 `FLAP_RPC_READ_URLS`、`FLAP_RPC_REALTIME_URLS`、`FLAP_RPC_HISTORY_URLS` 分别配置。各池只在本池内切换，历史空结果继续要求两个独立来源。HTTP 批量日志也走同样的校验路径。节点预算与 Four.meme 进程共享，批量按子请求数计费，容量不足显式报错且保留游标。配置和指标说明见根 README 的 1.11.0 节。
