...
FOURMEME_API_DIRECT_RETRY_SECONDS=300
...
FOURMEME_SCRAPLING_MAX_TEXT_BYTES=4000000
...
FOURMEME_SCRAPLING_HTML_LOAD_DOM=false
FOURMEME_SCRAPLING_HTML_DISABLE_RESOURCES=true
...
Four.meme API 兜底：`pool`、API 结构探针、OpenFour 模板和 public address 种子会先走普通 Node HTTP；只有遇到 403/429/Cloudflare 或历史同域退避时，才通过同一个 Scrapling sidecar 抓取 `four.meme/meme-api`。触发一次拦截后，默认 300s 内 API 直接走 sidecar，避免每个端点重复打出 403。GET 接口直接由 Scrapling 浏览器上下文打开，POST 接口在同源页面内执行 `window.fetch()`，不依赖本地手动验证 Chrome profile。
